...
def update_hand(hand, word):
    ...
    for letter in list(update_hand.keys()):
        if update_hand[letter] < 1:
...
def is_valid_word(word, hand, word_list):
    ...
    word = word.lower()
    letter_freq_word = get_frequency_dict(word)
    if '*' in word:
        possible_words = []
        for vowel in VOWELS:
        ...
        for possible_word in possible_words:
            in_word_list.append((possible_word in word_list))
    ...
    for letter in word:
        if not letter in hand:
            return False
        if letter_freq_word[letter] > hand[letter]:
            return False
    return True
#
...
def substitute_hand(hand, letter):
    ...
    if not letter in substitute_hand.keys():
        return substitute_hand
    new_letter = random.choice(list(set(VOWELS + CONSONANTS) - set(substitute_hand.keys())))
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(number_of_hands):
        ...
        display_hand(hand)
        if substitution:
            substitution_str = input('Would you like to substitute a letter? ')
            if substitution_str.lower() == 'yes':
        ...
        print()
        if replay:
            replay_str = input('Would you like to replay the hand? ')
            if replay_str.lower() == 'yes':
