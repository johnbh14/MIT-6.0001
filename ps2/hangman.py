...
def get_available_letters(letters_guessed):
    ...
    returns: string (of letters), comprised of letters that represents which letters have not
      yet been guessed.
    '''
    not_been_guessed = string.ascii_lowercase
    for letter in list(letters_guessed):
        not_been_guessed = not_been_guessed.replace(letter, '')
    return not_been_guessed
...
def hangman(secret_word):
    ...
    * After each guess, you should display to the user the 
    ...
    while guesses != 0:
        ...
        elif guessed_letter in list(secret_word):
            letters_guessed.append(guessed_letter)
            print('Good guess: ' + get_guessed_word(secret_word, letters_guessed))
        elif guessed_letter in get_available_letters(letters_guessed):
            letters_guessed.append(guessed_letter)
            guesses -= 1
...
def show_possible_matches(my_word):
    ...
    '''
    The below function will work without calling the 'match_with_gaps' function
    '''
    ...
    for other_word in wordlist:
        ...
        for i in range(len(my_word)):
            ...
            if my_word[i] == '_' and other_word[i] in my_word:
                #print('Failed Missing Letter Is Known Letter Req')
                break
            elif (my_word[i] == other_word[i] or my_word[i] == '_'):
                #print('Found Letter Match!')
                letter_matches.append(True)
            else:
                #print('Failed Letter Match')
                break
        if all(letter_matches) and len(letter_matches) == len(my_word):
            #print('Adding Word!')
            possible_matches.append(other_word)
    if len(possible_matches) > 0:
        print(' '.join(possible_matches))
